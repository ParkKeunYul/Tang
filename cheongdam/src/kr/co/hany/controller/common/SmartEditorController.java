...
public class SmartEditorController {
		...
		try { 
			if(editor.getFiledata() != null && editor.getFiledata().getOriginalFilename() != null && !editor.getFiledata().getOriginalFilename().equals("")) { 
				...
				if(cnt == 0) { 
					...
					String dftFilePath = Const.UPLOAD_ROOT;
					...
					return3 += "&sFileURL=/upload/editor/"+realFileNm; 
	...
	public void file_uploader_html5( HttpServletRequest request
		...
		try { 
			...
			if(cnt == 0) { 
				PrintWriter print = response.getWriter(); print.print("NOTALLOW_"+filename); print.flush(); print.close(); 
			} else { 
				...
				//파일 기본경로 _ 상세경로 
				//String filePath = dftFilePath + "editor" + File.separator + "editor" + File.separator +"multiupload" + File.separator;
				String filePath = Const.UPLOAD_ROOT+"editor/";
