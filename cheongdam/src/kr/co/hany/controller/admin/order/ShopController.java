...
public class ShopController extends AdminDefaultController{
	...
	@RequestMapping ("point_cancel.do")
	public @ResponseBody Map<String, Object> point_cancel( Map<String, Object> map
					  		 					 		  ,HttpServletResponse response
					  		 					 		  ,HttpSession session){
		Map<String, Object> rtn = PageUtil.procSet();
		try{
			...
			shopDao.update_point_cancel(param);
			shopDao.refundPoint(param);
			
			Date today = new Date();
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			
			rtn = PageUtil.procSuc();
			rtn.put("point_cancel_id", adminSession.get("a_id")+"/"+sdf.format(today));
		}catch (Exception e) {
			e.printStackTrace();
		}
		return rtn;
