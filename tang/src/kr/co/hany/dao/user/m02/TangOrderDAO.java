...
public class TangOrderDAO extends DefaultDAO{
	...
	public boolean saveCart(CommonMap param
		...
		try {
			...
			sqlSession.delete(NAMEPSACE+"del_cart_temp", param);
			...
			sqlSession.delete(NAMEPSACE+"del_temp", param);
	...
	public void  temp_yakjae( CommonMap param
		      				 ,List<Map<String, Object>> yakjae_list) {
		TransactionStatus status = txManager.getTransaction(new DefaultTransactionDefinition());
		try {
			...
			for(int i = 0; i<yakjae_list.size(); i++) {
				...
				info.put("mem_seqno", param.getString("mem_seqno"));
				sqlSession.insert(NAMEPSACE+"add_temp", info);
			}// for
	...
	public int check_temp(CommonMap param) {
		return (Integer)sqlSession.selectOne(NAMEPSACE+"check_temp", param);
	}
	
	
	@SuppressWarnings("unchecked")
	public List<Map<String, Object>> select_temp(CommonMap param)throws Exception{
		return sqlSession.selectList(NAMEPSACE+"select_temp", param);
	}
}
