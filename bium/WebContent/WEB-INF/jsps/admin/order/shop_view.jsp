		...
		$('#printBtn').click(function(){
			...
			var left2  = window.pageXOffset + parseInt( ( $(window).width()/2 )  - (width/2)  );
	 	    var top2   = window.pageYOffset + parseInt( ( $(window).height()/2 ) - (height/2) );
	  		...
	  		window.open ("/admin/order/shop/view_print.do?seqno=" + $('#seqno').val()+"&order_no="+$('#order_no').val(),"over_view_","toolbar=no, location=no, directories=no,resizable=no,left="+left2+",top="+top2+", width="+width+", height="+height+",scrollbars=yes")
	  		return;
			return false;
		});
		
		
		$('#cmsCancelBtn').click(function(){
			console.log('cms 결제 취소');
			
			if(!confirm('간편결제 결제금액을 취소하겠습니까?')){
				return false;
			}
			
			$.ajax({
				url : '/admin/order/shop/cms_cancel.do',
				data : $("#cms_cancel_frm").serialize(),        				
		        error: function(){
			    	alert('에러가 발생했습니다.\n관리자에 문의하세요.');
			    },
			    success: function(data){
...
<input type="hidden" name="seqno" id="seqno" value="${bean.seqno}" />
	...
	<table class="basic01">
		...
		<tbody>
						...
						<c:if test="${info.cancel_ing eq 'i'}">
							...
							<div style="display: none;" >
								...
								<form name="cms_cancel_frm" method="post" id="cms_cancel_frm">
									...
									<input type="text" name="MID" 			id="cms_MID"  value="<%=Const.NP_CMS_MID%>">
			...
			<tr <c:if test="${info.payment eq 'Card'}">style="display:none;"</c:if>>
				...
				<th>계산서/영수증</th>
				<td class="tdL">
					<c:if test="${info.payment eq 'Bank' || info.payment eq 'Cms' }">
