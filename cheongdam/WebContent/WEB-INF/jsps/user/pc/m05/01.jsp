...
<div id="container">
	...
	<div id="contents">
		...
		<div class="sub_gnb">
			...
			<p class="sg"><span>마이페이지</span><span>내 정보수정</span></p>
		</div>
					...
					<tr>
						...
						<td>
							...
							<div style="<c:if test="${info.info_fix ne 'Y'}">display:none;</c:if>">
								...
								<input type="checkbox" name="mailing" id="mailing" value="y" <c:if test="${info.mailing eq 'y' }">checked="checked"</c:if> style="vertical-align: middle;" > <span class="f13">이메일 수신 </span>
							</div>
							<c:if test="${info.info_fix ne 'Y'}">
								${info.email}
								<span class="t02">(전자세금계산서 발행용도)</span>
		...
		<!-- btnarea -->
		<div class="btn_area01" style="padding:20px 0 0 0;">
			<!-- <a href="#"><span class="cw h60">취소</span></a> -->
			<c:if test="${info.info_fix ne 'Y'}">
				<span class="t01" style="font-size: 20px;">비밀번호만 수정가능한 상태입니다<br/>정보 수정이 필요한경우 원외탕전 고객센터로 연락하세요.</span>
			</c:if>
			<br/>
