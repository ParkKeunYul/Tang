	...
	$(document).ready(function() {
		...
		$('#appOrderBtn').click(function () {
			...
			$(".part_check:checked").each(function() {
				...
				var sal_type = $('#sale_type_'+seqno).val();
				...
				if(sal_type != a_sal_type){
					alert('가맹점 상품은 일반 상품과 동시 결제 할 수 없습니다.');
					subCheck = false;
				}
				i++;
			});
			...
			<c:forEach var="list" items="${list}">
				...
				<tr id="list_${list.seqno}">
					<td>
						...
						<input type="hidden" id="sale_type_${list.seqno}" value="${list.sale_type}" />
					</td>
					<td class="L">
						...
						<div class="name">
							...
							<c:if test="${list.sale_type eq 2 }">
								<span style="color:#fb665f;">(가맹점)</span>
							</c:if>
	...
	<form action="02_order_app_cart.do" name="frm" id="frm" method="post">
		...
		<c:if test="${userInfo.member_level ne 0 && userInfo.member_level ne 1 }">
			<div class="btn_area01">
				<a href="02_order_app.do" id="appOrderBtn"><span class="cp h60">발송요청하기</span></a>
			</div>
		</c:if>
	</form>
	<!-- //btnarea -->
