	...
	<table class="basic01">
		...
		<tbody>
			...
			<tr>
				<th>공지글 등록여부</th>
				<td class="tdL">
					<select name="notice_yn" id="notice_yn" style="width: 90%;">
					<option value="n" <c:if test="${info.yak_status eq 'n' }">selected="selected"</c:if>>공지안함</option>
					<option value="y" <c:if test="${info.yak_status eq 'y' }">selected="selected"</c:if>>공지함</option>
				</select>
				</td>
			</tr>
			<tr>
				<th>카테고리</th>
				<td class="tdL">
					<select name=cate_nm id="cate_nm" style="width: 50%;">
						<option value="계약 및 신고" <c:if test="${info.cate_nm eq '계약 및 신고' }">selected="selected"</c:if>>계약 및 신고</option>
						<option value="주문 및 배송" <c:if test="${info.cate_nm eq '주문 및 배송' }">selected="selected"</c:if>>주문 및 배송</option>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="tdL">
					...
					<c:if test="${not empty add_bean.ori_name2 }">
						...
						<br/><a onclick="downloadBoard('${add_bean.ori_name2}', '${add_bean.re_name2}', '${add_bean.board_name}')"  href="#">${add_bean.ori_name2}</a>
			...
			<tr>
				...
				<td class="tdL">
					...
					<c:if test="${not empty add_bean.ori_name3 }">
						...
						<br/><a onclick="downloadBoard('${add_bean.ori_name3}', '${add_bean.re_name3}', '${add_bean.board_name}')"  href="#">${add_bean.ori_name3}</a>
