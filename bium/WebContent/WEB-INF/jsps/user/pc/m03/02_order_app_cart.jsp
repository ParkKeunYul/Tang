...
<div class="contents">
	...
	<c:choose>
		<c:when test="${userSession.member_level eq  4}">
			<ul class="submenu w25">
				<li><a href="/m03/01.do" class="">약속처방</a></li>
				<li><a href="/m99/01.do" class="">약속처방(가맹점)</a></li>
				<li><a href="/m03/02.do" class="sel">약속처방 보관함</a></li>
				<li><a href="/m03/03.do" class="">사전조제지시서 관리</a></li>
			</ul>
		</c:when>
		<c:otherwise>
			<ul class="submenu w33">
				<li><a href="01.do" class="">약속처방</a></li>
				<li><a href="02.do" class="sel">약속처방 보관함</a></li>
	...
	<form action="02_order_com_cart.do" name="frm" id="frm" method="post">
	<!-- 본문내용 -->
	<!-- orderview -->
			...
			<c:forEach var="list" items="${list}">
				...
				<tr>
					<td class="L">
						...
						<input type="hidden" id="goods_seq_${list.p_seq}" value="${list.p_seq}" />						
						<input type="hidden" id="goods_seq_ea_${list.p_seq}" value="${list.ea}" />
					</td>	
...
<!-- //배송정보 -->
	...
	<div class="btn_area01">
		...
		<c:if test="${userInfo.member_level ne 0 && userInfo.member_level ne 1 }">
			<a href="#" id="orderYakBtn"><span class="cp h60">주문하기</span></a>
		</c:if>
	</div>		
	<!-- //btnarea -->
	...
	<script>
		function cmsResult(){
			closeFancyBox();
			$("#frm").attr("action", "/m03/02_order_com_cart_cms.do");
			$('#frm').submit();
		}
		...
		$(document).ready(function() {
			...
			$("input:radio[name=ship_type_from]").click(function(){
				...
				var base_delivery_price     = parseInt( $('#delivery_price').val());
				
				var delivery_price = parseInt( $('#a_delivery_price').val());
