$(document).ready(function() {
  	...
  	$("#jqGrid").jqGrid({
  		...
  		colNames:[
  			'번호', '제품명',  '사이즈',  '가격' , '재고량', 
  			'설명' ,'이미지1', '이미지2', '이미지3' ,'임시컨텐츠'
  		],
  		colModel:[
  			{name:'seqno',		index:'seqno',			width:28,	align:"left" ,key: true , hidden:true},
  			{name:'sty_name',	index:'sty_name',		width:98,	align:"left"},
  			{name:'sty_size',	index:'sty_size',		width:68,	align:"center"
  				,editable:true
  			},
  			{name:'price',		index:'price',		width:58,	align:"right" 
  				,formatter: 'integer'
  				,formatoptions:{thousandsSeparator:","}
  				,editable:true
  			},
  			{name:'status',	index:'status',		width:58,	align:"center"
  				,editable:true 
  				,edittype:"select"
  				,editoptions:{
  					 value:"y:재고충분;n:재고부족"
  			...
  			{name:'contents',	index:'contents',	width:300,	align:"left"
  				,editable:true 
  				,edittype:"textarea"
  				,editoptions : {
 	  					 row :20
  			...
  			{name:'image1',	index:'image1',		width:100,	align:"center", formatter: getImgSrc},
  			{name:'image2',	index:'image2',		width:100,	align:"center", formatter: getImgSrc, hidden:true},
  			{name:'image3',	index:'image3',		width:100,	align:"center", formatter: getImgSrc, hidden:true},
  			{name:'sty_contents',		index:'sty_contents',		width:300,	align:"left" , hidden:true}
  		],
		pager: "#jqGridControl",
		viewrecords: true,
		autowidth: true,
		...
		loadComplete: function(data) {
		    ...
		    for (var i = 0; i < rows.length; i++){
		    	$("#jqGrid").setCell(rows[i] , 'sty_name', "" ,  {color:'blue',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경
		    	var contents = $("#jqGrid").getCell(rows[i],"contents");
		    	$("#jqGrid").setCell(rows[i] , 'sty_contents', contents); // 특정 cell 색상변경
		    }//for
		},
		onCellSelect : function(row,iCol,cellcontent,e){
...
function getImgSrc(cellValue, options, rowObject){
	...
	if(cellValue != '' && cellValue != null && cellValue !=  undefined){
		return "<img src='/upload/sty/"+row_img+"' width='100px;' height='100px;' />";
	}else{
		return '';
	}
}
