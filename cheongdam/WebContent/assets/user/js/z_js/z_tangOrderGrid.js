...
$(document).ready(function() {
	...
	$("#jqGrid").jqGrid({
		...
		loadComplete: function(data) {
			...
			if(mem_sub_grade == 2){
				$("#jqGrid").jqGrid('hideCol',["yak_danga","danga"]);
			}
		...
		onCellSelect : function(row,iCol,cellcontent,e){
			var data = $("#myDicGrid").jqGrid('getRowData', row);
			 ...
			 if(iCol == 1){
				 ...
				 $('#mydic_pop_detail').attr('href', '/m05/07_dictionary_popup.do?seqno='+data.seqno);
				 ...
				 $('#b_name').val(data.b_name);
				$.ajax({
					url : '/m05/07_mydic_yakjaeInfo.do',
					type: 'POST',
					data : {
						my_seqno : data.seqno,
					},
