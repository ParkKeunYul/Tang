...
<script>
	$(document).ready(function() {
		...
		$('#pointCancelBtn').click(function(){
			console.log('포인트 결제 취소');
			
			if(!confirm('고객의 포인트를 복구하겠습니까?')){
				return false;
			}
			
			$.ajax({
				url : '/admin/order/shop/point_cancel.do',
				data : $("#point_cancel_frm").serialize(),        
		        error: function(){
			    	alert('에러가 발생했습니다.\n관리자에 문의하세요.');
			    },
			    success: function(data){
			    	...
			    	if(data.suc){
			    		$("#jqGrid").trigger("reloadGrid");
			    		$('#point_cancel_info').html(data.point_cancel_id);
			    		$('#pointCancelArea').show();
			    		$('#pointCancelBtn').hide();			    		
			    	}
			    }   
			});
		...
		var point_cancel_id = $('#point_cancel_id').val();
		if(point_cancel_id != '' && point_cancel_id != null){
			$('#pointCancelBtn').hide();
			$('#pointCancelArea').show();			
		}
...
<input type="hidden" name="order_no" id="order_no" value="${info.order_no}" />
	...
	<table class="basic01">
		...
		<tbody>
			...
			<tr>
				...
				<%-- <td class="tdR"><strong><fmt:formatNumber value="${list.use_point}" pattern="#,###" /></strong>원</td> --%>
	...
	<table class="basic01">
		...
		<tbody>
			...
			<tr <c:if test="${info.payment eq 'Bank' || info.payment eq 'Point'}">style="display:none;"</c:if>>
				<th>거래번호</th>
					...
					<c:if test="${info.cancel_ing eq 'i'}">
						...
						<div style="display:none;" >
							...
							<form name="keyin_cancel_frm" method="post" id="keyin_cancel_frm">
								...
								<input type="text" name="CancelAmt" 	id="c_CancelAmt" value="${info.all_price - info.use_point}">
			...
			<tr <c:if test="${info.payment eq 'Card' || info.payment eq 'Point'}">style="display:none;"</c:if>>
				<th>영수증신청</th>
				<td class="tdL">
					<c:choose>
			...
			<c:if test="${info.use_point > 0}">
				<tr>
					<th><span style="color: blue;font-weight: 700;">결제금액 정보</span></th>
					<td class="tdL" colspan="3">
						<%-- 총 결제금액<span style="color: blue;font-weight: 700;font-size: 22px;"><fmt:formatNumber value="${info.all_price}" pattern="#,###" /></span>원
						<br/> --%>
						포인트 <span style="color: red;font-weight: 700;font-size: 22px;"><fmt:formatNumber value="${info.use_point}" pattern="#,###" /></span> 사용
						<c:if test="${info.payment eq 'Bank' }">
							<br/>무통장 입금금액 :   <span style="color: blue;font-weight: 700;font-size: 22px;"><fmt:formatNumber value="${info.all_price - info.use_point}" pattern="#,###" /></span>원
						</c:if>
						<c:if test="${info.payment eq 'Card' }">
							
							<br/>카드 결제금액 <span style="color: blue;font-weight: 700;font-size: 22px;"><fmt:formatNumber value="${info.all_price - info.use_point}" pattern="#,###" /></span>원
						</c:if>
						...
						<a href="#" id="pointCancelBtn"    class="btn03" style="vertical-align: top;">포인트 결제 취소</a>
							
						<div style="display: none;">
							<input type="text" id="point_cancel_id" value="${info.point_cancel_id}" />
							<form name="point_cancel_frm" method="post" id="point_cancel_frm">
								<input type="text" name="mem_seqno" 	id="mem_seqno" value="${info.mem_seqno}" />
								<input type="text" name="pp_seqno" 	id="pp_seqno" value="${info.order_no}" />
								<input type="text" name="point" 	id="point" value="${info.use_point}" />
								<input type="text" name="reason" 	value="주문건 취소로 인한 포인트 복구">
								<input type="text" name="c_order_no" 	i value="${info.order_no}" />
			                 </form>
		                 </div>
		                 
		                 <div style="display: none;" id="pointCancelArea">
							포인트 환급정보 : <span id="point_cancel_info">${info.point_cancel_id} / ${info.point_cancel_date}</span>
						</div>
	...
	<p><span class="viewtit">배송정보</span>
		<span class="txt02">
			(
				...
				<c:if test="${info.ship_type_from eq 2 }">청담원외탕전 -> 한의원</c:if>
				<c:if test="${info.ship_type_from eq 3 }">청담원외탕전 -> 고객</c:if>
