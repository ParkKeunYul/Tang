...
<%
	...
	*/
	...
	String payment_kind = StringUtil.objToStr(bean.get("payment_kind"), "Card");
	
	
	String encodeKey		= Const.NP_CMS_encodeKey;
	String MID       		= Const.NP_CMS_MID;
	...
	System.out.println("결제금액 01_add_order_cms frame   = "+ (Amt));
	
	
	String p_tot_price = bean.get("tot_price")+"";
	
	System.out.println("결제금액 01_add_order_cms frame  tot_price = "+ (p_tot_price));
	
	
	int  flagPrice = 0;
	if(!p_tot_price.equals(Amt)){
		flagPrice = -1;
	}
	
	System.out.println("flagPrice= "+ (flagPrice));
	...
	*/
	...
	String hashString    	  = sha256Enc.encrypt(ediDate + MID + Amt + encodeKey);								
...
<body style="margin: 0 0; padding: 0 0;">
	<div style="width: 789px;height: 505px;margin: 0 0; padding: 0 0;">
		<div  style="display: ;">
			<form action="https://payu.nicepay.co.kr/main.do" name="frm" id="frm" method="post"  accept-charset="euc-kr">>
			
				<input type="text" name="flagPrice" id="flagPrice"   value="<%=flagPrice %>" /> <br/>
			
				<!-- 산용카드 정보 -->
				
				<input type="text" name="MID" value="<%=MID%>" /> 상점ID<br />
				<input type="text" name="MallUserID" value="${userInfo.id}" /> 상점 고객ID<br />
				<input type="text" name="UserKey" value="${bean.cms_key}" /> 간편결제 사용자Key<br />
				<input type="text" name="AuthType" value="2"/> CharSet<br />
				<input type="text" name="BuyerName" value="${userInfo.name}"  /> 고객명<br />
				<input type="text" name="BuyerTel" value="${fn:replace(userInfo.handphone, '-', '')}" /> 고객 핸드폰<br />
				<input type="text" name="BuyerEmail" value="${userInfo.email}" /> 고객 Email<br />
				
				<input type="text" name="RcvName" value="${bean.r_name}" /> 수취인 명<br />
				<input type="text" name="RcvTel" value="${bean.r_tel01 }${bean.r_tel02 }${bean.r_tel03 }" /> 수취인 전화번호<br />
				<input type="text" name="RcvPostNo" value="${bean.r_zipcode }" /> 배송지 우편번호<br />
				<input type="text" name="RcvAddr1" value="${bean.r_address01 }" /> 배송지 주소1<br />
				<input type="text" name="RcvAddr2" value="${bean.r_address02 }" /> 배송지 주소2<br />
				
				
				<input type="text" name="Moid" value="<%=moid%>"  /> 주문번호<br />
				<input type="text" name="EdiDate" value="<%=ediDate%>"/> 전문 생성일시<br />
				<input type="text" name="Amt" id="Amt" value="<%=Amt%>" /> 결제금액<br />
				<input type="text" name="GoodsName" value="<%=goodsName%>" /> 상품명<br />
				<input type="text" name="ReturnUrl" id="ReturnUrl" value="http://biumherb.co.kr/cmsResult.do" /> 간편결제 인증결과 수신URL<br />
				<input type="text" name="SignData" value="<%=hashString%>" /> 해쉬값<br />
				 <!-- <input type="text" name="ApproveMode" value="API" /> ApproveMode <br /> -->
				<input type="text" name="CharSet" value="utf-8"/> CharSet<br />
				...
				<a href="#" onclick="nicepayStart();">1111111111111111</a>
			</form>
		</div>
	</div>
</body>
...
$(document).ready(function() {
	...
	var Amt = $('#Amt').val();
	if(Amt< 1000){
		alert("1000원 미만은 계좌 간편 결제를 이용할 수 없습니다.");
		parent.closeFancyBox();
	}else{
		console.log(document.domain);
		var nowDoamin = document.domain;
		
		$('#ReturnUrl').val('http://'+nowDoamin+'/cmsResult.do');
