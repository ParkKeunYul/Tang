...
public class MyCartDAO extends DefaultDAO{
	...
	public boolean del_cart(CommonMap param) {
		try {
			...
			sqlSession.delete(NAMEPSACE+"del_cart_yak", param);
			return true;
		}catch (Exception e) {
			e.printStackTrace();
			return false;
		...
		try {
			...
			for(int i = 0; i < cart_seqno.length ; i++) {
				...
				if(i == 1) {
					...
					param.put("order_delivery_price", (delivery_price+member_sale));
					param.put("member_sale"         , member_sale);
					param.put("order_total_price"   , delivery_price);
				}
				/*
	...
	@SuppressWarnings("unchecked")
	public List<Map<String, Object>> sub_id_list(CommonMap param)throws Exception{
		return sqlSession.selectList(NAMEPSACE+"sub_id_list", param);
	}
