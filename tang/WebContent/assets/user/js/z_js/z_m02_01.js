...
var a_jusu_price      = 12000;
var a_jeunglyu_price  = 8000;
var a_balhyo_price    = 20000;
var a_jaetang_price   = 7000;

var a_sti_price       = 0;
var a_yakjae_price    = 0;
var a_delivery_price  = 4000;
var a_delivery_plus   = 2000;
...
function InitPriceInfoSet(){
	...
	var c_pack_ea = parseInt(objToStr($('#c_pack_ea').val() ,0));
	var c_box_ea  = getBoxEa();
	...
	var l_delivery_price = a_delivery_price + ((c_box_ea-1) *a_delivery_plus)
	
	$('#order_delivery_price').val(l_delivery_price );
	$('#order_delivery_price_txt').html( comma(l_delivery_price+'') );
	...
	var tot_pojang_price = a_pouch_price+(a_box_price*c_box_ea)+a_sti_price + ( a_one_pack_pirce * c_pack_ea );
...
function getBoxEa(){
	...
	var c_box_ea =  parseInt( c_pack_ea/ 60);
	if( (c_pack_ea % 60) != 0 ){
		c_box_ea = c_box_ea +1;
	}
	$('#c_box_ea').val(c_box_ea);
	//console.log('getBoxEa =', c_box_ea);
...
$(document).ready(function() {
	...
	$("#c_pouch_type").change(function() {
		...
		var c_pack_ea       = parseInt(objToStr($("#c_pack_ea").val(),0));
		var c_box_ea        = getBoxEa();
		var tot_pouch_price = parseInt(a_pouch_price) + (a_one_pack_pirce * c_pack_ea);
		
		console.log('a_pouch_price = ',a_pouch_price);
		console.log('c_pack_ea = ',c_pack_ea);
		console.log('tot_pouch_price =', tot_pouch_price);
		
		$('#order_pojang_price').val(tot_pouch_price+(a_box_price*c_box_ea)+a_sti_price);
		$('#order_pojang_price_txt').html( comma(( tot_pouch_price+(a_box_price*c_box_ea)+a_sti_price )+'') );
	...
	/*
	var a_jusu_price      = 8000;
	var a_jeunglyu_price  = 8000;
	var a_balhyo_price    = 20000;
	*/
	
	$('#c_tang_check14').change(function() {
		if($("#c_tang_check14").is(":checked")){
			$('#order_suju_price').val(a_jeunglyu_price);
			$('#order_suju_price_txt').html( comma(a_jeunglyu_price+'') );
	...
	$('#c_tang_check15').change(function() {
		if($("#c_tang_check15").is(":checked")){
			$('#order_suju_price').val(a_balhyo_price);
			$('#order_suju_price_txt').html( comma(a_balhyo_price+'') );
	...
	$('#c_tang_check16').change(function() {
		if($("#c_tang_check16").is(":checked")){
			$('#order_suju_price').val(a_jaetang_price);
			$('#order_suju_price_txt').html( comma(a_jaetang_price+'') );
	...
	$("#delItemBtn").click(function() {
	    ...
	    for (var i = 0; i < rows.length; i++){						    	
		 ...
		 for (var i = 0; i < rows.length; i++){
			$("#jqGrid").setCell(rows[i] , 'sort_seq_temp', i+1 ,  {});
	 	 }// for
	...
	$("#preOrderGrid").jqGrid({
  		...
  		colModel:[
  			...
  			{name:'handphone',		index:'handphone',	width:63,	align:"center"  , hidden:true},
  		],
		...
		onSelectRow : function(row,cellcontent,e){
			...
			setTimeout(function(){
				/*
				 console.log('onSelectRow11 =', param);
				 console.log('onSelectRow11 data=', data);
				 */
	...
	$("#patientDetailGrid").jqGrid({
  		...
  		colNames:[
  			...
  			'b_name'          , 'c_tang_check14'    , 'c_tang_check15', 'c_tang_check16'
  		],
  		colModel:[
  			{name:'seqno',				index:'seqno',				width:28,	align:"center" ,key: true , hidden:true},
  			...
  			{name:'c_tang_check14',		index:'c_tang_check14',			width:10,	align:"center"  , hidden:true},
  			{name:'c_tang_check15',		index:'c_tang_check15',			width:10,	align:"center"  , hidden:true},
  			{name:'c_tang_check16',		index:'c_tang_check16',			width:10,	align:"center"  , hidden:true},
		...
		onCellSelect : function(row,iCol,cellcontent,e){
			...
			if(iCol == 3 || iCol == 4){
				...
				if(data.c_tang_check14 == 'y'){
					$('#c_tang_check14').prop("checked", true);
					$('#c_tang_check14').attr("disabled", false);
				...
				if(data.c_tang_check15 == 'y'){
					$('#c_tang_check15').prop("checked", true);
					$('#c_tang_check15').attr("disabled", false);
				...
				if(data.c_tang_check16 == 'y'){
					$('#c_tang_check16').prop("checked", true);
