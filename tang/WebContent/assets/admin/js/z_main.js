	...
	$(document).ready(function() {
		$("#tangGrid").jqGrid({
	  		...
	  		colModel:[
	  			...
	  			{name:'order_no',	index:'order_no',	width:128,	align:"center"  ,sortable : false },
	  			{name:'s_name',		index:'s_name',	width:350,	align:"left"    ,sortable : false},
	  			{name:'name',		index:'name',		width:100,	align:"center"  ,sortable : false},
	  			{name:'w_name',		index:'w_name',		width:100,	align:"left"    ,sortable : false ,hidden:true},
	  			
	  			{name:'d_to_name',	index:'d_to_name',	width:100,	align:"left"    ,sortable : false ,hidden:true},		  			
	  			...
	  			{name:'han_name',			index:'han_name',			width:150,	align:"left"  ,sortable : false,editable:false},
	  			...
	  			{name:'cash_bill_nm',	index:'cash_bill_nm',		width:50,	align:"center"  ,sortable : false ,hidden:true},
	  			{name:'order_date2',		index:'order_date',			width:190,	align:"center"  ,sortable : false},
	  			...
	  			{name:'delivery_no',	index:'delivery_no',	width:120,	align:"center"  ,sortable : false ,hidden:true},
	  			{name:'c_more_tang_nm',	index:'c_more_tang',	width:60,	align:"center"  ,sortable : false ,hidden:true},
			...
			loadComplete: function(data) {
				var rows = $("#tangGrid").getDataIDs(); 				
			    for (var i = 0; i < rows.length; i++){
			        
			    	var payment = new Number($("#tangGrid").getCell(rows[i],"payment"));
			    	if(payment == 1){
			    		$("#tangGrid").setCell(rows[i] , 'payment', "" ,  {color:'blue',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경	
			    	}else{
			    		$("#tangGrid").setCell(rows[i] , 'payment', "" ,  {color:'red',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경
			    	}
			    					    
			    	var payment_kind = $("#tangGrid").getCell(rows[i],"payment_kind");				   
			    	if(payment_kind == 'Card'){
			    		$("#tangGrid").setCell(rows[i] , 'payment_kind_nm', "" ,  {color:'blue',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경	
			    	}else{
			    		$("#tangGrid").setCell(rows[i] , 'payment_kind_nm', "" ,  {color:'red',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경
			    	}
			    	
			    	
			    	var order_ing = new Number($("#tangGrid").getCell(rows[i],"order_ing"));
			    	if(order_ing == 1){
			    		$("#tangGrid").setCell(rows[i] , 'order_ing', "" ,  {color:'blue',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경
			    	}else if(order_ing == 3){
			    		$("#tangGrid").setCell(rows[i] , 'order_ing', "" ,  {color:'green',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경
			    	}else{
			    		$("#tangGrid").setCell(rows[i] , 'order_ing', "" ,  {color:'black',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경
			    	}
			    	
			    	$("#tangGrid").setCell(rows[i] , 's_name', "" ,  {'text-decoration': 'underline',color:'#396B93',weightfont:'bold',cursor: 'pointer'}); // 특정 cell 색상변경
			    	
			    	//$("#tangGrid").setCell(rows[i] , 's_name', "" ,  {color:'#396B93',weightfont:'bold',cursor: 'pointer', text-decoration: 'underline'}); // 특정 cell 색상변경
			    	
			    	var bunch = $("#tangGrid").getCell(rows[i],"bunch");
			    	if(bunch != 'n'){
			    		var s_name = $("#tangGrid").getCell(rows[i],"s_name");				    		
			    		$("#tangGrid").setCell(rows[i],"s_name", s_name+"<font color='red'>[묶음]</font>")
			    	}				
			    	
			    	var cancel_ing =  $("#tangGrid").getCell(rows[i],"cancel_ing");
			    	if(cancel_ing == 'i' || cancel_ing == 'y'){
			    		var s_name = $("#tangGrid").getCell(rows[i],"s_name");
			    		$("#tangGrid").setCell(rows[i],"s_name", s_name+"<font color='red'>[취소요청건]</font>")
			    	}
			    }

			},
			onCellSelect : function(row,iCol,cellcontent,e){					 
				 var record = $("#tangGrid").jqGrid('getRowData', row);
		...
		$("#tang_form").dialog({
           ...
           // position: 'center',
            ...
            close : function(){            
            //	$("#tangGrid").trigger("reloadGrid");
            	$("#tang_form").html("");
            },
            open: function(event, ui) { 
            	$('.ui-widget-overlay').bind('click', function(){
            		$("#tang_form").dialog('close'); 
            	});
            },
            buttons: {	                
                "닫기": function () {
		...
		$("#shopGrid").jqGrid({
	  		...
	  		colModel:[
	  			...
	  			{name:'order_date',			index:'order_date',			width:180,	align:"center"    ,sortable : false},
	  			{name:'order_ing',			index:'order_ing',			width:100,	align:"center"    ,sortable : false
	  				,editable:false 
	  				,edittype:"select"
	  				,editoptions:{
