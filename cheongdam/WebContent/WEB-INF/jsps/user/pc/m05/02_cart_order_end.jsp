				...
				<c:if test="${bean.payment_kind eq 'Bank' }">
					<table class="cart_end">
						...
						<tbody>
							<tr>
								<th>총 결제금액</th>
								<td><span class="fc01_t b"><fmt:formatNumber value="${bean.result_price}" pattern="#,###원" /></span></td>
							</tr>
							<tr>
								<th>입금계좌</th>
			...
			<table class="cart_list">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${list.d_type eq 1 }">청담원외탕전  → 한의원</c:when>
								<c:when test="${list.d_type eq 4 }">청담원외탕전  → 고객</c:when>
								<c:when test="${list.d_type eq 3 }">한의원 → 고객</c:when>
								<c:when test="${list.d_type eq 6 }">방문수령</c:when>
								<c:when test="${list.d_type eq 6 }">직접입력</c:when>
							</c:choose>
